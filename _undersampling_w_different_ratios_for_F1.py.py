...
for ratio in ratios:
    ...
    # Save the best model
    dump(best_model, f"f1_best_random_forest_model_ratio_{ratio}.joblib") """


# Modeli yükleme
loaded_model = load('f1_best_random_forest_model_ratio_0.1.joblib')
...
if 'mpl_toolkits.mplot3d' in sys.modules:
    del sys.modules['mpl_toolkits.mplot3d']

warnings.filterwarnings("ignore", message="Unable to import Axes3D.")

# Matplotlib ve Axes3D'yi kullanabilmek için
...
feature_importances = pd.Series(loaded_model.feature_importances_, index=X_train.columns).sort_values(ascending=False)
