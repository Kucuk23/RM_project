...
def SEDI_fitness(individual):
    # Model parametrelerini ayarla (örneğin, ağaç sayısı ve derinlik)
    n_estimators = max(1, abs(int(individual[0])))
    max_depth = max(1, abs(int(individual[1])))
    ...
    tp_count = ((y_test_scaled == 1) & (predictions == 1)).sum()
    tp_total = (y_test_scaled == 1).sum()

    correct_true_count = np.sum(np.logical_and(predictions == 1, y_test_scaled.values == 1))
    correct_false_count = np.sum(np.logical_and(predictions == 0, y_test_scaled.values == 0))
    incorrect_true_count = np.sum(np.logical_and(predictions == 1, y_test_scaled.values == 0))
    incorrect_false_count = np.sum(np.logical_and(predictions == 0, y_test_scaled.values == 1))

    # SEDI hesaplama
    cr = correct_false_count 
    fa = incorrect_false_count
    h = correct_true_count
    m = incorrect_true_count
...
toolbox.register("evaluate", SEDI_fitness)
...
for ratio in ratios:
    ...
    toolbox.register("evaluate", lambda ind: SEDI_fitness(ind))
    ...
    # Save the best model
    dump(best_model, f"2_sedi_best_random_forest_model_ratio_{ratio}.joblib")


"""
# Modeli yükleme
loaded_model = load('2_sedi_best_random_forest_model_ratio_0.05.joblib')
